		...
		SubShader
		{
			Pass
			{
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					fixed4 diffuseColor = _DiffuseColor * col;
					...
					fixed4 specular = smoothstep(0.5f,0.55f,pow(saturate(dot(reflectDir, eyeDir)), 20)) * _LightColor0;

					fixed4 ads = (1 - intensity) * ambient + diffuse + specular;
